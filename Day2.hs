...
run :: IntCodes -> IntCodes -> IntCodes
--Note, if the program can overwrite locations it hasn't yet executed, this will not work
run codes (99:_) = codes
run codes (opCode:a:b:c:instr) = run codes' instr
    where
        codes' = execute f codes a b c
        f | opCode == 1 = (+)
          | opCode == 2 = (*)
